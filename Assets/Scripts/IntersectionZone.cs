...
public class IntersectionZone : MonoBehaviour
{
    ...
    // The exit direction IntersectionDetector decided was "correct" for this
    // intersection when it was built. Recorded purely so ValidateBuild() can
    // cross-check it against a fresh recomputation before the player starts.
    public Vector2Int CorrectExit { get; private set; }
    public bool HasCorrectExit { get; private set; }

    public void SetCorrectExit(Vector2Int exit)
    {
        CorrectExit = exit;
        HasCorrectExit = true;
    }
